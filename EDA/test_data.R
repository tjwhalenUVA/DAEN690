...
#Query----
t.lean <- dbGetQuery(con,'select * from test_lean')
t.content <- dbGetQuery(con,'select * from test_content')
dbDisconnect(con)
...
left_join(t.content %>% select(id, `published-at`),
          t.lean %>% select(id, bias),
          by='id') %>%
    mutate(Year = lubridate::year(`published-at`),
           Year = ifelse(is.na(Year), 2021, Year),
           PublishDate = ifelse(Year == 2021, 'No', 'Yes'),
